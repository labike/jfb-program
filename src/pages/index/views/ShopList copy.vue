...
<template>
	<section class="shop-list"  v-if="navList">
        <div class="nav-bar">
		...
		<swiper :style="'height:'+swiperHeight+'px'"
			...
			<block v-for="(swi,s) in navList" :key="s">
				<swiper-item>
					...
					<scroll-view scroll-y v-if="swi.shopList"
...
export default {
    ...
    methods: {
        ...
        updataShopList(index, page) {
            return new Promise((resolve,reject) => {
                ...
                apiGetRecommends({
                    ...
                    let shopList = _current.shopList.slice(0)
        ...
        _formatNavList() {
            let that = this;
            let navList = that.allIndysty.filter(indysty => indysty.index_show && indysty.index_show === '1')
            navList.map(indysty => {
                indysty.shopList = null
                return indysty
            })
            return navList
        }
    ...
    onLoad() {
        ...
        that.navList = that._formatNavList()
